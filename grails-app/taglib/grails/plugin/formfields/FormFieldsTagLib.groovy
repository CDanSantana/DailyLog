...
class FormFieldsTagLib {
	...
	def all = { attrs ->
		...
		try {
			...
			if (domainClass) {
            out << '<div class="container"><div class="row"><div class="fieldcontain">'
	...
	private CharSequence renderAssociationInput(Map model, Map attrs) {
		...
		if (manyToMany) {
			...
			/*if (!model.required)*/ attrs.noSelection = ["": "Selecione"]
			attrs.value = model.value?.id
	...
	private CharSequence renderOneToManyInput(Map model, Map attrs) {
		...
		//buffer << '<ul>'
        buffer << '<div class="container"><div class="row"><div class="fieldcontain">'
		def persistentProperty = model.persistentProperty
		def controllerName
		def shortName
		if (persistentProperty instanceof Association) {
    ...
    def paginateStatus = { attrs, body ->
        if (!attrs.from == null) {
            throwTagError("Tag [paginationStatus] ausencia de parametro obrigatório [from]")
        }
        if (!attrs.total == null) {
            throwTagError("Tag [paginationStatus] ausencia de parametro obrigatório [total]")
        }
        if (attrs.from instanceof grails.orm.PagedResultList || attrs.from != null) {
            def totalCount = attrs.total
            out << '<div class="pagination" id="paginationId" role="alert" aria-live="polite" aria-relevant="all">'
            if (totalCount > 0) {
                def offset = params.offset ? params.offset?.toInteger() : 1
                def max = params.max ? params.max?.toInteger() : 10
                if (totalCount <= max) {
                    out << "Exibindo <b>${totalCount}</b> de <b>${totalCount}</b> encontrados"
                } else {
                    out << "Exibindo de <b>${(offset)}</b> a <b>${(offset?.toInteger() + params.max?.toInteger())}</b> de <b>${totalCount}</b> encontrados"
                }
            } else {
                out << '<h6>Não existem resultados a serem exibidos</h6>'
            }
            out << '</div>'
        }else{
            out << ''
